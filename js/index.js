...
//touch sounds
const switchThemeAudio = new Audio(
	'https://www.fesliyanstudios.com/play-mp3/641'
);
const taskDoneAudio = new Audio();
taskDoneAudio.src =
	'/assets/sounds/mixkit-select-click-1109 AETrim1668595932780.wav';
	...
	if (taskText) {
		TodoList.innerHTML = `
          <div class="Todo-list-sub-cont">
            <div class="cont">
              <button class="check-logo-cont ${
								Element && Element.complete ? 'check-logo-cont-toggle' : ''
							}">
...
const totalTaskLeft = function () {
	...
	todoList.forEach((ele) => {
		if (!ele.children[0].children[1].classList.contains('complete')) count++;
	});
	countList.forEach((ele) => {
		ele.innerText = `${count} items left`;
	});
...
const removeActiveClass = function () {
	const info = document.querySelectorAll('.Todo-status button');
	info.forEach((ele) => ele.classList.remove('active'));
};
